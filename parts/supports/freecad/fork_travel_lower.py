# -*- coding: utf-8 -*-
"""
Lower leg clamp — fori passanti lungo Y sulle alette; tab magnete = stessa forma dello stantuffo (speculare −Y).
"""
...
R_in_lo = LOWER_OD_MM / 2.0 + DIAMETRIC_SLACK / 2.0
...
def build_upper_half() -> Part.Shape:
    u = _half_ring_upper(R_in_lo, R_out_lo, LOWER_CLAMP_H, GAP).fuse(
        _junction_lugs(R_in_lo, R_out_lo, LOWER_CLAMP_H, GAP, upper=True)
    )
    try:
...
def build_lower_half() -> Part.Shape:
    u = (
        _half_ring_lower(R_in_lo, R_out_lo, LOWER_CLAMP_H, GAP)
        .fuse(_junction_lugs(R_in_lo, R_out_lo, LOWER_CLAMP_H, GAP, upper=False))
        .fuse(
            ftc.sensor_tab_unified(
                R_in_lo,
                R_out_lo,
                LOWER_CLAMP_H,
                GAP,
...
def main() -> None:
    ...
    if doc is None:
        doc = App.newDocument("ForkTravelLowerLeg")

    a = doc.addObject("Part::Feature", "HalfUpper_semicerchio")
    a.Shape = build_upper_half()
    a.Placement = App.Placement(App.Vector(-100.0, 0, 0), App.Rotation())

    b = doc.addObject("Part::Feature", "HalfLower_magnete")
    b.Shape = build_lower_half()
    b.Placement = App.Placement(App.Vector(100.0, 0, 0), App.Rotation())
