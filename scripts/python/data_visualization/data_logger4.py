...
class MapVisualizer(tk.Frame):
    ...
    Also provides a button to open a web map (satellite tiles) using folium.
    """
    def __init__(self, master, app, **kwargs):
        super().__init__(master, **kwargs)
        self.app = app
    ...
    def _build_ui(self):
        ...
        # New button: open web map with satellite tiles
        webmap_btn = tk.Button(control_frame, text="Open web map", command=self.open_web_map)
        webmap_btn.pack(side=tk.LEFT, padx=5)
    ...
    def open_web_map(self):
        """
        Open a web browser with a satellite-style map showing the path (lat/lon).
        Uses folium with Esri World Imagery as tile source.
        """
        if self.df is None:
            messagebox.showwarning("No data", "No data loaded.")
            return
        ...
        # Drop rows with NaN lat/lon if any
        sub_df = self.df[[lat_col, lon_col]].dropna()
        if sub_df.empty:
            messagebox.showwarning("No valid data", "No valid lat/lon points to plot on the map.")
            return

        lat_vals = sub_df[lat_col].to_numpy()
        lon_vals = sub_df[lon_col].to_numpy()

        # Center of the path
        lat_center = float(np.mean(lat_vals))
        lon_center = float(np.mean(lon_vals))

        # Create folium map with Esri World Imagery (satellite tiles)
        m = folium.Map(location=[lat_center, lon_center], zoom_start=14, tiles=None)
        ...
        folium.PolyLine(coords, weight=3, color="red").add_to(m)
